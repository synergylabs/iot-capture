...
void led_blink(int gpio, int delay, int count)
{
	...
	gpio_level =  gpio_get_level((gpio_num_t) gpio);
	for (int i = 0; i < count; i++) {
		gpio_set_level((gpio_num_t) GPIO_OUTPUT_NOTIFICATION_LED, 1 - gpio_level);
		vTaskDelay(delay / portTICK_PERIOD_MS);
		gpio_set_level((gpio_num_t) GPIO_OUTPUT_NOTIFICATION_LED, gpio_level);
		vTaskDelay(delay / portTICK_PERIOD_MS);
	}
}
...
void change_led_state(int noti_led_mode)
{
	...
	switch (noti_led_mode)
	{
		...
		case LED_ANIMATION_MODE_SLOW:
			gpio_level =  gpio_get_level((gpio_num_t) GPIO_OUTPUT_NOTIFICATION_LED);
...
void gpio_init(void)
{
	...
	io_conf.pull_down_en = (gpio_pulldown_t) (BUTTON_GPIO_RELEASED == 0);
	io_conf.pull_up_en = (gpio_pullup_t) (BUTTON_GPIO_RELEASED == 1);
	gpio_config(&io_conf);
	...
	gpio_set_level((gpio_num_t) GPIO_OUTPUT_COLORLED_R, 0);
	gpio_set_level((gpio_num_t) GPIO_OUTPUT_COLORLED_G, 0);
	gpio_set_level((gpio_num_t) GPIO_OUTPUT_COLORLED_B, 0);
	gpio_set_level((gpio_num_t) GPIO_OUTPUT_COLORLED_0, 0);
}
