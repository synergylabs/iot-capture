...
# Both the ports should be the same
stream_port = 81
http_port = 80

stream_status_lock = Lock()
stream_status = 0
...
class Message:
    def __init__(self, message):
        ...
        self.length = None
        self.payload = b''
        (self.type, self.length) = unpack('II', message)
        self.type = socket.ntohl(self.type)
        self.length = socket.ntohl(self.length)

    def construct_message(self):
        if self.length:
            message = pack('II', socket.htonl(self.type), socket.htonl(self.length)) + self.payload
        else:
            message = pack('II', socket.htonl(self.type), socket.htonl(self.length))
        return message
...
class CameraHTTPServer:
    ...
    def start_http_server(self):
        server = HTTPServer(('', self.http_port), CameraServerRequestHandler)
        server.serve_forever()

class CameraServerRequestHandler(BaseHTTPRequestHandler):

    def read_payload(self, header, conn=None):
        if not conn:
        ...
        if header.length:
            ...
            while to_read > 0:
                size_min = min (MAX_BUFFER_SIZE, to_read)
                buf = conn.recv(size_min)
                data = data + buf
                to_read -= len(buf)
            print(f"read_payload: total length: {len(data)}")
            return data

    def read_data(self, conn=None):
        if not conn:
            global device_connection
            conn = device_connection
        header = b''
        ...
        print(f"Recv header: type: {header.type}, length: {header.length}")
        length = header.length
        payload = self.read_payload(header, conn)
        return length, payload

    def empty_socket(self, conn=None):
    ...
    def do_GET(self):
        if self.path == '/':
            ...
            _, payload = self.read_data()
            self.send_response(200)
            self.send_header('Content-type','text/html')
            self.send_header('Content-Encoding', 'gzip')
            self.end_headers()
            if 'ov3660' in str(payload):
                # send index_ov3660
                self.wfile.write(bytes(index_ov3660_html_gz))
                pass
            else:
                # send index_ov2640
                self.wfile.write(bytes(index_ov2640_html_gz))
                pass
        elif self.path == '/status':
            # import pdb; pdb.set_trace()            
            print("Processing request for /status")
            self.empty_socket()
            message = Message(bytes(HEADER_LENGTH))
            message.type = STATUS
            msg = message.construct_message()
            ...
            print("Processing request for /control")
            s = self.path
            import urllib.parse
            req = urllib.parse.parse_qs(s[s.index('?')+1:])
            # sanity check for var and val length
            len_val = len(req['val'][0])
            len_var = len(req['var'][0])
            if len_val >= 32 or len_var >= 32:
                print("ERROROROROR too long val or var")
            else:
                self.empty_socket()
                message = Message(bytes(HEADER_LENGTH))
                ...
                message.payload = req['var'][0].encode('utf-8') + bytes(32-len_var) + req['val'][0].encode('utf-8') + bytes(32-len_val)
                msg = message.construct_message()
                # import pdb; pdb.set_trace()            
                device_connection.sendall(msg)
            print("Finish process control message")
            pass
        elif self.path == '/capture':
            print("Processing request for /capture")
            self.empty_socket()
            message = Message(bytes(HEADER_LENGTH))
            ...
            length, payload = self.read_data()
            if length > 0:
                self.send_response(200)
                self.send_header('Content-type', 'image/jpeg')
                self.send_header("Content-Disposition", "inline; filename=capture.jpg")
                self.send_header('Content-length', str(length))
                self.end_headers() 
                print(f"Writing {len(payload)} bytes payload")
                self.wfile.write(payload)
                self.wfile.flush()
            print("Finish process capture message")
            # if length > 1024: 
            #     sleep(5)
            pass
    ...
    def finish(self):
        print("actually finish!!!")
    
    def start_stream_server(self):
        # stream_thread = Thread(target=self.handle_streaming, args=())
        # stream_thread.start()
        self.handle_streaming()

    def handle_streaming(self):
        (stream_conn, address) = device_socket.accept()
        ...
        while True:
            try:
                ...
                self.empty_socket(stream_conn)
                message = Message(bytes(HEADER_LENGTH))
                message.type = STREAM_CONTENT_1
                msg = message.construct_message()           
                stream_conn.sendall(msg)
                _, payload = self.read_data(stream_conn)
...
class Camera_Driver:
    ...
    def run_http_server(self, http_port, dev_conn):
        ...
        camera_http = CameraHTTPServer(http_port)
        http_thread = Thread(target=camera_http.start_http_server, args=())
        http_thread.start()

    def run_stream_server(self, stream_port):
        stream_http = CameraHTTPServer(stream_port)
        stream_thread = Thread(target=stream_http.start_http_server, args=())
        stream_thread.start()
    
    def accept_device_connection(self):
        # while True:
        (conn, address) = device_socket.accept()
        self.device_connection = conn
        self.run_http_server(http_port, conn)
        self.run_stream_server(stream_port)
...
def main():
    ...
    camera_driver = Camera_Driver(int(sys.argv[1]), int(sys.argv[2]))
