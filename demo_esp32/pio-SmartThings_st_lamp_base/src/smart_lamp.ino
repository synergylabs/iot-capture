...
static void color_led_onoff(int32_t state)
{
	...
	{
		gpio_set_level((gpio_num_t) GPIO_OUTPUT_COLORLED_R, (smartlamp_color_red > 127));
		gpio_set_level((gpio_num_t) GPIO_OUTPUT_COLORLED_G, (smartlamp_color_green > 127));
		gpio_set_level((gpio_num_t) GPIO_OUTPUT_COLORLED_B, (smartlamp_color_blue > 127));
	}
}
						  ...
						  iot_cap_cmd_data_t *cmd_data, void *usr_data)
{
	...
	// For measuring end-to-end latency
	IOT_EVENT *switch_evt = st_cap_attr_create_string("feedback", "switch-on", NULL);
	st_cap_attr_send(handle, evt_num, &switch_evt);
