...
class Capture_Driver:
    def __init__(self, listening_port, ip_subnet):
        ...
        self.device_socket.bind(('192.168.%d.1' % ip_subnet, listening_port))
    ...
    def start_web_server(self):
        ...
        serversocket.bind(('0.0.0.0', 8088))
        ...
        while True:
            ...
            if clientsocket:
                ...
                # Manually disconnecting the client for test purposes
                if TESTING:
                    time.sleep(5)
                    self.client_list[self.client_counter - 1] = None
...
def main():
    if len(sys.argv) <= 2:
        ...
        return -1
    capture = Capture_Driver(int(sys.argv[1]), int(sys.argv[2]))
